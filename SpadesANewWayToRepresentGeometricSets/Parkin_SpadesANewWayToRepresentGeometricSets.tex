...
If $\dim\V$ is finite, then for every geometric set that can be represented by an element $E\in\G$,
there exists an $r$-blade $B_r\in\G$ such that $\gd(E)=\gd(B_r)$.
\end{lem}
\begin{proof}
By Lemma~\ref{lem_reduce_inf_vec_set}, any set of vectors generating the geometric
set represented by $E$ can be reduced to a finite subset $\{b_i\}_{i=1}^r$ of
$r$ linearly independent vectors.  Now let $B_r=\bigwedge_{i=1}^r b_i$.
\end{proof}

The next lemma becomes useful in the case that our vector space $\V$ is of infinite dimension.
...
\begin{lem}\label{lem_convert_spade_to_blade_when_lin_indep}
...
It is instructive to examine an alternative proof of Lemma~\ref{lem_convert_spade_to_blade_when_lin_indep}.  Notice that if $\{m_i\}_{i=1}^r$
...
Of course, what is unsatisfactory about Lemma~\ref{lem_convert_spade_to_blade_when_lin_indep} is
that it is not always the case that $0\neq\bigwedge_{i=1}^r m_i$.

Find recursive algorithm for the conversion?
