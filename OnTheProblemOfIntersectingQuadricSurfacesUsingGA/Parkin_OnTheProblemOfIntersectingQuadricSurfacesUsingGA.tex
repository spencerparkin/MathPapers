...
replaced, if done in a consistent manner, by either the inner product symbol ``$\cdot$'' or
the outer product symbol ``$\wedge$.''
...
% hyp: if B is an irreducible canonical form and p(x1)^...^p(xk) is a factorization
% of it, then the system generated by, for all i, p(xi).B, is a completely determined sys.

\section{Returning To Our Original Approach}

% The sections and organization of this paper should undergo some refactoring if I can push a result through.

Still not ready to give up, let us introduce yet another lemma to help us in our quest.
While Lemma~\ref{lma_irreducible_form_exists} dealt with existence, this one
...
such that $\gh(B)=\gh\left(\bigwedge_{i=1}^k p(x_i)\right)$, then there is no
irreducible form of $B$ of a grade less than $k$, nor greater than $k$.
\end{lem}
\begin{proof}

\end{proof}

Armed with Lemma~\ref{lma_unique_grade_of_irreducible_form}, we are justified in
...
\end{equation}
which is a blade of grade 5 in our algebra.\footnote{Choosing any 5 points $\{x_i\}_{i=1}^5$ on
...
that does not depend on such knowledge, let's move forward unashamed
...
% This paper's efficacy may be found in what we've learned from the attempt,
% and so the following questions are left open to the thoughtful reader.

% Can we rotation a reducible intersection into a reducible canonical form?
