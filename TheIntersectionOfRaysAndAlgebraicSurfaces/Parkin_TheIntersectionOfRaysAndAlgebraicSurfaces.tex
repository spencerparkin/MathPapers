...
Admittedly, this result may be more of theoretical interest than of having any practical
application to the problem of ray casting algebraic surfaces.
Nevertheless, equation \eqref{equ_result} has been used to formulate
the coeficients of a quadric equation which can then be easily solved
by the quadratic formula.  A variety of quadrics were ray-taced using this method and
can be seen in Figure~\ref{}.  The quadratic equation used may be written as
\begin{equation*}
f(x) + \lambda v\cdot\nabla f(x) + \lambda^2 v_1\wedge v_2\cdot B.
\end{equation*}

As can be easily seen, each of the ceoficients are easy to compute, given $f$ and $\nabla f$.
This is not quite the case, however, when we consider the cubic equation
\begin{equation*}
f(x) + \lambda v\cdot\nabla f(x) + \frac{\lambda^2}{2}\nabla_v(v\cdot\nabla f(x)) +
\lambda^3 v_1\wedge v_2\wedge v_3\cdot B.
\end{equation*}
Here, all but the third coeficient is easily computed.  Interestingly, we may rewrite
$\nabla_v(v\cdot\nabla f(x))$ as $v\cdot\nabla_v(\nabla f(x))$, in which form,
approximating the derivative may be a bit easier.  In all cases, a coeficient can
be computed as an approximation of the following limit.
\begin{equation*}
\frac{T_i(x)}{\lambda^i} = \lim_{\delta\to 0}\frac{f(x+\delta v)-f(x)}{\delta^i}.
\end{equation*}
...
% Of course, anything of degree 5 or higher must be solved using
% numerical methods, are there is no algebraic solution to polynomials
% of such degrees.
