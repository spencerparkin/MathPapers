...
A = \bigcap_{k=1}^r A_k,
\end{equation*}
where each $A_k$ is the set of all solutions in each $\alpha_i$ to equation $k$, given by
\begin{equation*}
...
\begin{defn}
We define the function $\gd:\G\to P(\F^n)$ as
\begin{equation*}
...
\begin{lem}
For any element $E\in\G$, the set $g(E)$ is a geometric set.
\end{lem}
\begin{proof}
\end{proof}

\begin{lem}
If $B_r$ is an $r$-blade and $M_r$ is a spade with $\langle M_r\rangle_r\neq 0$, then we have
\begin{equation*}
\gd(B_r)=\gd(M_r).
\end{equation*}
\end{lem}
