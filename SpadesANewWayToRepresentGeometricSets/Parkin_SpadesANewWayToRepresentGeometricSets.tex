...
\end{equation}
The integer $r$ may be finite or infinite.
\end{defn}
...
Note that it should be immediately clear from Definition~\ref{def_geo_set} that the intersection of any two geometric sets is geometric.
...
is not so easily proven (or disproven) and of such considerable importance to the theory of spades in geometric algebra, that, if proven, would
...
An alternative for blades can be found in \cite[p. 88]{Doran03}.}
...
then $\gd(E)$ is a geometric set by Lemma~\ref{lem_solution_intersection}.  (Recall that the intersection
of any two geometric sets is geometric.)

The case $k=0$ is trivial.  Letting $k>0$, it is clear by equation \eqref{equ_a_dot_Br_sum_of_blades} that
\begin{equation*}
0 = p(x)\cdot\langle E\rangle_k = \sum_i (p(x)\cdot v_i)V_i,
\end{equation*}
where each $V_i$ is a blade of grade $k-1$.  We now see that the set of all solutions $x$
to this equation gives us a geometric set by Lemma~\ref{lem_geo_set_lin_dep}.
\end{proof}

\begin{lem}
Setup $p$.
Then, for every algebraic set $S$, there exists a blade $B_r\in\G$ representative of it.
\end{lem}
\begin{proof}
By the Hilbert Basis Theorem (see \cite[p. 204]{Garrity13}), there exists a finite
set of $r$ vectors $\{v_i\}_{i=1}^r\subset\V$ such that $S$ is given by equation \eqref{equ_geo_set_def}.
It's now trivial to show that this can be reduced to any linearly independent set that spans this set of vectors.
Now form that blade.
\end{proof}

\begin{lem}
If $B_r$ is an $r$-blade and $M_r$ is a spade with $\langle M_r\rangle_r=\lambda B_r$, then we have
\begin{equation*}
\gd(B_r)=\gd(M_r).
\end{equation*}
\end{lem}
This is dumb.  Spades are dumb.  Why do we even care?

So any multivector represents a geometric set.  Do blades represent a strict subset of all geometric sets?  Do spades?
Are the set of all geometric sets represented by blades the same as spades?
