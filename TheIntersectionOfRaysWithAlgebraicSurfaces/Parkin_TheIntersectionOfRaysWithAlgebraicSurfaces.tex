...
This result is applied to the problem of casting rays through algebraic surfaces.
\end{abstract}
...
Letting $f:\R^n\to\R$ be any polynomial function in $n$ variables and up to degree $m$, it
...
\end{equation}
where $B$ is an $m$-vector of our geometric algebra with $\nvai_i\cdot B_j=0$ for all
integers $i\in[1,m]$ and all $m$-blades $B_j$ if we were to write $B=\sum B_j$,
...
on account that $\nvai_i\cdot B_j=0$.\footnote{Use (2.7) of \cite{Parkin13} to get \eqref{equ_ray_polynomial_unexpanded} in
the case that $B$ is an $m$-blade, then show that \eqref{equ_ray_polynomial_unexpanded} holds when $B$ is an $m$-vector.}
