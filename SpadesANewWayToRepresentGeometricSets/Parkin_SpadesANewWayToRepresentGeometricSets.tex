...
Finally returning now to the concept of spade rank, the proof of the following lemma is pieced together
...
itself to its own desirable properties, which we choose to think about a particular problem and what we know
...
Given a blade, if we want the associated spade, we must factor it, if it is not already orthogonalized.
Converting from a spade to a blade, however, is much easier.  By Lemma~\ref{lem_spade_rank_iff_lin_indep},
we simply take the highest grade part of the spade's expansion.  
