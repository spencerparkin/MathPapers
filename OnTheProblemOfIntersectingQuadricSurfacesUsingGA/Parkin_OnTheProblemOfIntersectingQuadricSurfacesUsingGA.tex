...
\end{lem}
\begin{proof}
This proof is false!!!  The partitioning step doesn't work.  Consider $(e_1+e_2)\wedge e_1\wedge e_2=0$.
...
\begin{algorithmic}
\Function{Reduce}{$B$}
	\State $x\gets$\Call{FindAnyPoint}{$B$}
	\If{$p(x)\wedge B=0$}
		\State\Return $p(x)\,\wedge\,$\Call{Reduce}{$p(x)\cdot B$}
	\EndIf
	\State\Return $1$
\EndFunction
\end{algorithmic}
...
of flat points, all geometries of the conformal model are irreducible.  (This is not true.
Consider tangent points.)
