...
% Can we prove this is a blade of grade r-1?
...
spade of highest possible rank is no greater than $r$, showing that by equation \eqref{equ_a_dot_Br_sum_of_blades}
the grade of $s$ can be diminished by at most $r$.  Similarly, if $s<r$, the grade $r$ can be diminished by at most $s$.
...
B_r\cdot C_s &= \langle B_rC_s\rangle_{s-r}\nonumber \\
 ...
 &= B_{r-1}\cdot(b_r\cdot C_s).
\end{align}
We need to justify why we need $r\leq s$ here, but I'm too tired to figure it out at the moment.
